...
async function api(path: string, options: RequestInit = {}, retry = true): Promise<any> {
  const current = readSession(), headers = new Headers(options.headers)
...
async function refreshOrders(){try{await load()}catch(e){error.value=e instanceof Error?e.message:'โหลดออเดอร์ไม่สำเร็จ'}}
...
async function uploadImage(product:Product,event:Event){const input=event.target as HTMLInputElement;const file=input.files?.[0];if(!file)return;uploadingId.value=product.id;error.value='';message.value='';try{const form=new FormData();form.append('image',file);await api(`/merchant/products/${product.id}/image`,{method:'POST',body:form});message.value=`เพิ่มรูป ${product.name} แล้ว`;await load()}catch(e){error.value=e instanceof Error?e.message:'อัปโหลดไม่สำเร็จ'}finally{uploadingId.value=null;input.value=''}}
...
function logout(){if(orderTimer)window.clearInterval(orderTimer);saveSession(null);store.value=null;products.value=[];orders.value=[];error.value='';message.value=''}
onMounted(async()=>{if(!readSession())return;busy.value=true;try{await load();startOrderRefresh()}catch{logout()}finally{busy.value=false}})
...
<main class="merchant-page">
  ...
  <template v-else>
    ...
    <div v-if="tab==='orders'" class="order-toolbar"><span>อัปเดตอัตโนมัติทุก 8 วินาที</span><button class="secondary" :disabled="busy" @click="refreshOrders">รีเฟรชตอนนี้</button></div>
    ...
    </form><div class="card product-admin-list"><h2>เมนูทั้งหมด</h2><div v-if="products.length===0" class="empty">ยังไม่มีเมนูอาหาร</div><article v-for="product in products" :key="product.id" class="product-admin-row"><img v-if="product.image_url" :src="product.image_url" class="menu-thumb" alt=""/><div @click="edit(product)"><strong>{{product.name}}</strong><small>{{product.is_tracking?`เหลือ ${product.stock}`:'ไม่ติดตามสต็อก'}}</small></div><b>฿{{Number(product.price).toFixed(0)}}</b><label :class="['image-button',{uploading:uploadingId===product.id}]">{{uploadingId===product.id?'กำลังอัปโหลด…':product.image_url?'เปลี่ยนรูป':'เลือกไฟล์รูป'}}<input type="file" accept="image/jpeg,image/png,image/webp" :disabled="uploadingId!==null" @change="uploadImage(product,$event)"/></label></article></div></section>
    <section v-else class="report-section"><div class="card report-head"><label>วันที่<input v-model="reportDay" type="date" @change="loadReport"/></label><div v-if="report" class="metrics"><div><span>ยอดขาย</span><b>฿{{report.revenue.toFixed(2)}}</b></div><div><span>ต้นทุน</span><b>฿{{report.cost.toFixed(2)}}</b></div><div><span>ค่าใช้จ่าย</span><b>฿{{report.expenses_total.toFixed(2)}}</b></div><div class="net"><span>รายรับสุทธิ</span><b>฿{{report.net_income.toFixed(2)}}</b></div></div></div>
      <div class="card sales-detail"><div class="sales-detail-head"><div><h2>ยอดขายแยกตามเมนู</h2><small>นับจากออเดอร์ที่ทำเสร็จแล้ว {{report?.completed_orders||0}} ออเดอร์</small></div><b>รวม ฿{{Number(report?.revenue||0).toFixed(2)}}</b></div><div v-if="!report?.product_sales?.length" class="empty">วันนี้ยังไม่มีรายการขายสำเร็จ</div><div v-else class="sales-table"><div class="sales-row sales-header"><span>เมนู</span><span>ขายแล้ว</span><span>ยอดขาย</span><span>คงเหลือ</span></div><div v-for="item in report.product_sales" :key="item.product_id||item.name" class="sales-row"><strong>{{item.name}}</strong><span>{{item.quantity}} ชิ้น</span><b>฿{{Number(item.sales).toFixed(2)}}</b><span :class="{low:item.is_tracking&&item.stock<=5}">{{item.is_tracking?`${item.stock} ชิ้น`:'ไม่ติดตาม'}}</span></div></div></div>
      <form class="card" @submit.prevent="addExpense"><h2>บันทึกค่าใช้จ่าย</h2><label>รายการ<input v-model.trim="expenseDescription" required/></label><label>จำนวนเงิน<input v-model.number="expenseAmount" type="number" min="0.01" step="0.01" required/></label><button class="primary">บันทึก</button></form><div class="card"><h2>ค่าใช้จ่ายวันนี้</h2><div v-if="!report?.expenses.length" class="empty">ยังไม่มีค่าใช้จ่าย</div><div v-for="item in report?.expenses" :key="item.id" class="expense-row"><span>{{item.description}}</span><b>฿{{Number(item.amount).toFixed(2)}}</b></div></div></section>
  </template>
</main>
</template>
